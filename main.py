...
#SELF DEFINED OBJECT
#---------------------------------------------------------------
DATE = 612#date of FILE, for storage
META_NAME = 'model0'
CKPT_NAME = 'ckpt0'
DATA_DIR = '/home/zt/zt_datas/19_05_12/'
DATA_FILE = 'aandt_pro10w0512.csv'
RELOAD_MODLE_DIR = './train0512/ckpt0512_0/'
...
EXAMPLE_X_DIR = 'TRAIN_TEST_DATA/example_x.pkl'
EXAMPLE_Y_DIR = 'TRAIN_TEST_DATA/example_y.pkl'
#---------------------------------------------------------------
def load_arg():
    ...
    paser.add_argument("--save_mdn_params", type=bool,
                      default=True, help="save mdn params or not")
...
def main():
    ...
    if TrainIndicator:
        ...
        with tf.Session() as sess:
            ...
            train_writer = tf.summary.FileWriter('LOG/'+ args.model_type+ '/'+str(DATE)+ '/',sess.graph)
            ...
            for i in range(args.epoch):
                ...
                if test_cost_mean < best_test_cost:
                    ...
                    model.saver.save(sess, 'TRAINED_MODEL/'+ args.model_type+ '/'+ str(DATE)+ '/best_model.ckpt', global_step=i+1)
    ...
    if not TrainIndicator:
        ...
        with tf.Session(graph=restore_graph) as restore_sess:
            ...
            for test_num in range(total_test_num):
                ...
                past_lacation_frame.to_csv('RESULTS/'+ args.model_type+ '/'+ str(DATE)+ '/past_location_'+ str(test_num)+ '.csv', mode='w', index = None)
                ...
                pred_ground_frame.to_csv('RESULTS/'+ args.model_type+ '/'+ str(DATE)+ '/pred_ground_'+ str(test_num)+ '.csv', mode='w', index = None)
                if MdnParamsIndicator:
                    mean_stdframe = pd.DataFrame(result_of_7car, columns = mean_std_label)
                    mean_stdframe.to_csv('RESULTS/'+ args.model_type+'/'+ str(DATE)+'/mean_str_'+ str(test_num)+ '.csv', mode='w', index = None)
